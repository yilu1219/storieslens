...
function evaluateLaunchReadiness({ root, mediaStorageStatus = {}, renderWorkerReady = false, pdfRendererReady = false, env = process.env } = {}) {
  ...
  const required = [
    ...
    gate("otp-delivery", "Verified account access", hasEmailDelivery || enabled(env.INVITE_CODE_AUTH_ENABLED), "Connect Resend email delivery, a private email/SMS webhook, or high-entropy invitation-code access for a deliberately closed cohort."),
    gate("registration-access", "Verified registration access", !enabled(env.REGISTRATION_INVITE_REQUIRED) || invitesReady, "Keep public email registration open, or add a hashed invitation code for every open region when REGISTRATION_INVITE_REQUIRED=true."),
    ...
    gate("real-person-uploads", "Real-person photo governance", !enabled(env.REAL_PERSON_PHOTO_UPLOADS_ENABLED) || (enabled(env.REAL_PERSON_PHOTO_REQUIRE_ACCOUNT) && enabled(env.REQUIRE_PERSONAL_PHOTO_CONSENT) && enabled(env.ACCOUNT_DELETION_ENABLED)), "If personal-photo stories are enabled, require an adult-owned account, recorded consent and tested deletion."),
